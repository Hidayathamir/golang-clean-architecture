...
func createAddressViaAPI(t *testing.T, token string, contactID int64, requestBody model.CreateAddressRequest) model.AddressResponse {
	t.Helper()

	bodyJSON, err := json.Marshal(requestBody)
	assert.Nil(t, err)

	req := httptest.NewRequest(http.MethodPost, fmt.Sprintf("/api/contacts/%d/addresses", contactID), strings.NewReader(string(bodyJSON)))
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("Accept", "application/json")
	req.Header.Set("Authorization", bearerToken(token))
