...
- `internal/`: Clean architecture core.
  ...
  - `usecase`: Application services orchestrating business logic. Always split by domain.
  ...
  - `dependency_injection`: Application wiring and infrastructure setup.
  - `provider`: Setup for various infrastructure providers (DB, Redis, AWS, etc.).
  - `config`: Type-safe application configuration with getter methods.
  - `mock`: Autogenerated test doubles—do not hand-edit; regenerate instead.
...
- Application configuration is managed via `config.json` and loaded into a typed `Config` struct in `internal/config`. Access values via getter methods (e.g., `Cfg.GetWebPort()`).
